...
@Component({
  ...
  template: `
    <div class="category-page content-animate-enter">
      <div class="page-header">
        <h1 class="page-title">
          <span class="material-symbols-outlined">folder</span>
          {{ 'category.title' | t }}{{ categoryName }}
        </h1>
        <p class="page-subtitle">{{ posts.length }}{{ 'category.posts' | t }}</p>
      </div>
        ...
        @for (post of posts; track post.slug) {
          <a [routerLink]="'/posts/' + post.slug" class="archive-item">
            ...
            @for (tag of post.frontmatter.tags; track tag) {
              <span class="item-chip">{{ tag }}</span>
            }
          </a>
        } @empty {
          <div class="empty-state">
            <p>{{ 'category.empty' | t }}</p>
          </div>
        }
      </div>
    </div>
